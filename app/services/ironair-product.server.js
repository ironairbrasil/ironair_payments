...
export async function getIronAirPublicProduct() {
  ...
  const response = await fetch(`${storeOrigin()}/products/${encodeURIComponent(handle)}.js`, {
    headers: { Accept: "application/json" },
  });

  if (!response.ok) {
    throw new Error(`Não foi possível carregar o Iron Air da Shopify (${response.status}).`);
  }
  ...
  return {
    ...
    images: (product.images || []).map((image) => image.startsWith("//") ? `https:${image}` : image),
